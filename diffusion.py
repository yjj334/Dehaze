...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample_loop_d(self, x_lr, mask_0, h_hat, continous=False):
        ...
        for i, j in zip(reversed(seq), reversed(seq_next)):
            ...
            xt_next_temp = (x_lr + 1) / 2
            xt_next_temp = xt_next_temp * 2 - 1
            xtemp = xt_next_temp + x0_t  
    ...
    def p_losses(self, x_in, noise=None):
        ...
        x_recon, res_depth = self.denoise_fn(
            torch.cat([x_in['SR'], x_in['Degmap'], x_noisy], dim=1), t.float())
        ...
        loss_depth = self.loss_func(res_depth, x_in['Depth'])
        # loss_mask2 = self.loss_func(updated_mask2, res_map)
        return loss + loss_depth * 0.5
